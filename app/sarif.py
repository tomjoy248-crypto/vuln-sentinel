...
def _make_rule_id(finding: dict) -> str:
    ...
    return f"VS-{str(vuln_type).upper()}"
...
def _finding_to_sarif_result(finding: dict, rule_index_map: dict) -> dict:
    ...
    if url:
        ...
        parameter = finding.get("parameter", finding.get("location", {}).get("target", "")) or "N/A"
        if finding.get("parameter") or finding.get("location", {}).get("target"):
            location_entry["logicalLocations"] = [
                        ...
                        "locations": [
                            {
                                "location": {
                                    ...
                                    "message": {
                                        "text": json.dumps(evidence, ensure_ascii=False)[:1000]
                                    },
                                }
                            }
                        ]
...
def _build_sarif_rules(findings: list[dict]) -> tuple[list[dict], dict[str, int]]:
    ...
    for finding in findings:
        ...
        rule: dict[str, Any] = {
            "id": rule_id,
            "name": str(vuln_type).replace("_", " ").title()[:100],
            "shortDescription": {"text": finding.get("name", vuln_type)[:200]},
            "fullDescription": {"text": finding.get("summary", finding.get("description", ""))[:1000]},
            "helpUri": f"https://cwe.mitre.org/data/definitions/{cwe.replace('CWE-', '')}.html" if cwe else "",
            "defaultConfiguration": {
                "level": _SEVERITY_TO_SARIF_LEVEL.get(finding.get("severity", "medium"), "warning"),
            },
            "properties": {
                "tags": [owasp] if owasp else [],
                "precision": "high" if finding.get("confidence_level", "高") == "高" else "medium",
                "security-severity": _severity_to_score(finding.get("severity", "medium")),
            },
        }
        if cwe:
...
def export_to_sarif(
    scan_data: dict,
    tool_name: str = "Vuln Sentinel",
    tool_version: str = "11-S",
) -> dict:
    ...
    for finding in findings:
        try:
            results.append(_finding_to_sarif_result(finding, rule_index_map))
        except Exception as exc:
            logger.warning("SARIF result conversion failed for finding: %s", exc)
...
def import_from_sarif(sarif_data: dict) -> list[dict]:
    ...
    for run in runs:
        ...
        rule_map = {index: rule for index, rule in enumerate(rules)}
        rule_map_by_id = {rule.get("id", ""): rule for rule in rules}
        ...
        for result in results:
            ...
            if code_flows:
                ...
                if thread_flows:
                    ...
                    if flow_locs:
                        evidence_text = (
                            flow_locs[0].get("location", {}).get("message", {}).get("text", "")
                        )
                        if evidence_text:
                            try:
