...
_register(FixTemplate(
    vuln_type="ssrf",
    platform="express",
    code=("// 使用代理 + 禁止请求 169.254.169.254\n"
...
_register(FixTemplate(
    vuln_type="csrf",
    platform="flask",
    code=("# Flask-WTF 自动处理 CSRF Token\n"
...
_register(FixTemplate(
    ...
    platform="nginx",
    code=("location ~ /(\\.env|\\.git|\\.svn|backup\\.sql|\\.bak) {\n"
          "    deny all;\n"
          "    return 403;\n"
          "}"),
...
_register(FixTemplate(
    vuln_type="sensitive_path",
    platform="apache",
    code=("<LocationMatch \"/(\\.env|\\.git|\\.svn|backup\\.sql|\\.bak)\">\n"
          "    Require all denied\n"
          "</LocationMatch>"),
...
def generate_remediation_plan(
    ...
    legacy_fixes: Dict[str, List[Dict[str, Any]]] = {
        "nginx": [], "apache": [], "express": [], "flask": [],
        "spring_boot": [], "cloudflare": [], "nodejs": [], "python": [],
    }
    ...
    return {
        ...
        "summary": {
            ...
            "platforms_available": list({p for plan in plans for p in plan["fixes"].keys()}),
            "detected_platform": detected,
        },
    }
