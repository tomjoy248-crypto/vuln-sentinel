...
class SensitiveConfigExposureDetector(BaseVulnDetector):
    ...
    async def detect(self, context: ScanContext) -> list[Finding]:
        ...
        probes: list[tuple[str, str, str, re.Pattern[str]]] = [
            ...
            ("/application-prod.yaml", "生产 Spring 配置文件暴露", "high", re.compile(r"(spring:|datasource:|password:|secret:|jwt|oauth|redis|mongodb)", re.I)),
            ...
            ("/docker-compose.yaml", "Docker Compose 配置暴露", "high", re.compile(r"(services:|environment:|password|secret|token|db_|redis|postgres|mysql)", re.I)),
            ...
            ("/settings.yml", "应用 YAML 配置暴露", "high", re.compile(r"(secret:|password:|token:|client_secret:|database:|redis:)", re.I)),
            ("/config.yml", "应用 YAML 配置暴露", "high", re.compile(r"(secret:|password:|token:|client_secret:|database:|redis:)", re.I)),
            ...
            ("/secrets.yml", "密钥 YAML 文件暴露", "high", re.compile(r"(secret:|password:|token:|client_secret:|private_key:|access_key:)", re.I)),
            ("/secrets.yaml", "密钥 YAML 文件暴露", "high", re.compile(r"(secret:|password:|token:|client_secret:|private_key:|access_key:)", re.I)),
            ("/kubeconfig", "Kubernetes 配置文件暴露", "high", re.compile(r"(clusters:|users:|contexts:|client-certificate|client-key|token:)", re.I)),
            ("/.kube/config", "Kubernetes 配置文件暴露", "high", re.compile(r"(clusters:|users:|contexts:|client-certificate|client-key|token:)", re.I)),
            ...
            ("/helm/values.yaml", "Helm Values 配置暴露", "medium", re.compile(r"(password:|secret:|token:|apiKey:|clientSecret:|database)", re.I)),
