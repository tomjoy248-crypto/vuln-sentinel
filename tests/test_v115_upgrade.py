...
确认所有面向用户的版本标识都已从 V11.4 → V11.6,
并且 V11.6 新增的能力(LLM/auto-patrol/trusted domains/AI 顾问优化)都还在。
"""
import re
...
def test_settings_app_version_is_11_5():
    """Settings.app_version 应为 11.6"""
    assert M.settings.app_version == "11.6"


def test_settings_app_title_is_v11_5():
    """Settings.app_title 应为 漏洞哨兵 V11.6"""
    assert M.settings.app_title == "漏洞哨兵 V11.6"


def test_api_version_endpoint_returns_11_5():
    """/api/version 应返回 version=11.6"""
    client = TestClient(M.app)
    r = client.get("/api/version")
    assert r.status_code == 200
    body = r.json()
    assert body.get("version") == "11.6", f"got {body.get('version')}"
    assert "V11.6" in body.get("title", "")
...
# ============================================================
# 2) index.html 用户可见标识必须都是 V11.6
# ============================================================
def test_index_html_title_is_v11_5():
    html = open(str(ROOT / "static/index.html")).read()
    assert "<title>漏洞哨兵 V11.6" in html
    # 离线 /api/version 返回的 title 也得是 V11.6
    assert '漏洞哨兵 V11.6 (离线演示模式)' in html


def test_index_html_meta_description_is_v11_5():
    html = open(str(ROOT / "static/index.html")).read()
    assert 'name="description" content="漏洞哨兵 V11.6' in html
...
def test_index_html_user_facing_v11_5_strings():
    ...
    html = open(str(ROOT / "static/index.html")).read()
    # 进化页面 h3
    assert ">漏洞哨兵 V11.6<" in html
    # 我的页面段落
    assert "<strong>漏洞哨兵 V11.6</strong>" in html
    # AI 顾问话术: "你好！我是漏洞哨兵 V11.6"
    assert "我是漏洞哨兵 V11.6" in html
    # 评分公式回复提到 V11.6
    assert "V11.6 评分公式" in html
    # 版本更新回复提到 V11.6
    assert "V11.6 主要改进" in html


def test_index_html_offline_version_11_5():
    """离线模式 /api/health, /api/version 返回 11.5"""
    html = open(str(ROOT / "static/index.html")).read()
    assert 'version: "11.6-offline"' in html
    assert 'version: "11.6"' in html
    assert 'build_time: "2026-06-25"' in html


# ============================================================
# 3) V11.6 新增能力仍然存在(没在升级过程中丢)
# ============================================================
