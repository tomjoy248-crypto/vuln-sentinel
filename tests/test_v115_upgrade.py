...
def test_settings_app_version_is_11_s():
    """Settings.app_version 应为 V12"""
    assert M.settings.app_version == "V12"
...
def test_api_version_endpoint_returns_11_s():
    """/api/version 应返回 version=V12"""
    client = TestClient(M.app)
    r = client.get("/api/version")
    assert r.status_code == 200
    body = r.json()
    assert body.get("version") == "V12", f"got {body.get('version')}"
...
def test_index_html_offline_version_11_s():
    """离线模式 /api/health, /api/version 返回 V12"""
    html = open(str(ROOT / "static/index.html")).read()
    assert 'version: "V12"' in html
