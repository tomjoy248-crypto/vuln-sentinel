"""
V12 单元测试
覆盖：认证 / 工具函数 / WAF 检测 / 修复生成 / FastAPI 端点 / V12 新增能力
"""
import json
import os
import sys
...
# V12: analyze_security 已改为 async，提供同步包装器供测试调用
...
def test_index_html_served():
    ...
    # V12 标题校验
    assert "V12" in r.text


# ============== V12 新增能力：Public demo / Simulate fix / Verify fix diff / History delete ==============
...
def test_simulate_fix_uses_real_severity():
    """V12：simulate-fix 用真实 severity 字段，不再退化。"""
    findings = [
...
def test_history_delete_real():
    """V12：DELETE 真正清空历史，返回 deleted 数。"""
    r = client.post("/api/login", json={"username": "demo", "password": "demo123"})
    if r.status_code != 200:
        pytest.skip("demo account unavailable")
    tok = r.json()["token"]
...
# ============== V12：analyze_security 输出 summary ==============
