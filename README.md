...
漏洞哨兵把流程收敛为三步：**输入目标 → 查看结果 → 按建议修复**。当前版本已面向正式交付场景优化，适合作为上线前的基础自查、复测验证和修复闭环工具；对不在检测范围内的漏洞类型会明确标注，并通过审计页、工单页和复扫结果把流程串起来。
...
- **扫出来对不对**：高置信度结果更可信；低置信度和被防护设备干扰的结果会降权并标为待复核。
- **离真正专业产品还有多远**：还在产品化中后期，主流程已通，但更深的漏洞覆盖、误报压制和规则校准还要继续。

## 变现方式

- **按积分收费**：用户购买套餐后按扫描、复测和深度检测消耗积分。
- **按项目收费**：给客户做一次性站点体检、复测和报告交付。
- **按团队收费**：团队协作、审计留痕、导出、复核、权限分层作为付费能力。
...
| **审计闭环** | 通过审计页、工单和复扫记录串联“发现 → 修复 → 复核”流程 |
...
| 版本 | 价格 | 包含内容 |
|---|---:|---|
| **体验包** | ￥9.90 | 20 credits，适合小批量试用 |
| **标准包** | ￥69.90 | 120 credits，适合日常扫描 |
| **专业包** | ￥299.00 | 600 credits，适合高频使用 |
| **企业包** | ￥999.00 | 2400 credits，适合团队与规模化场景 |
