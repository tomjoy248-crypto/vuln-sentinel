...
- **扫出来对不对**：高置信度结果更可信；低置信度和被防护设备干扰的结果会降权并标为待人工复核。
...
- **增加真实样本回测**：重点覆盖带 WAF、CDN、登录态、重定向、软 404 的站点，持续压低误报和漏报。

- **给结果加置信度和证据等级**：把“确定”“较可能”“需人工复核”分层展示，避免把推测项包装成事实。

- **把复核提示做更明显**：对“未能验证”“证据不足”“受站点防护影响”的结果，前台直接提示人工复核。

- **强化关键识别**：继续加强登录态、重定向、WAF、软 404、通用错误页的识别与交叉验证。
...
- **通用错误页**：很多站点会返回统一模板页，看起来像漏洞，实际上只是 404/500/拦截页。

- **软 404**：页面状态码是 200，但内容其实是“页面不存在”或“资源已下线”。

- **CDN / WAF 干扰**：响应头、状态码、页面内容会被中间层改写，影响判断。

- **模板页伪装**：站点统一模板过于像真实业务页面，容易误判为有效证据。

- **单次请求不足**：很多结论必须靠二次验证、不同路径交叉验证或多策略确认。
...
| 类别 | 当前判断 | 说明 |
|---|---|---|
| 安全响应头 / Cookie / CORS / SSL | **能扫** | 结果相对稳定，适合对外收费和交付。 |
| 敏感路径 / 过时组件 / 开放重定向 | **能扫，但要看证据** | 有一定误报控制，需结合页面内容和重复验证。 |
| 软 404 / WAF / CDN 干扰站点 | **扫不稳** | 需要更强的交叉验证和“待复核”标识。 |
| 登录态相关、强交互业务流 | **扫不全** | 受权限、状态、流程、验证码影响，必须保留人工复核。 |
| 复杂业务逻辑漏洞 | **暂时不做核心卖点** | 这类问题黑盒覆盖率不稳定，容易产生漏报。 |
...
- **可以说**：我们能做常见 Web 安全风险的自动扫描、证据展示、修复建议和复测验证。

- **可以说**：结果会分成“已验证 / 可能存在 / 待人工复核”，帮助团队快速定位优先级。

- **可以说**：工具适合做基础安全体检、交付前复测和持续巡检。

- **不要说**：我们能发现所有漏洞、100% 准确、完全替代人工渗透测试。

- **建议说法**：这是一个“快速发现常见问题 + 输出证据 + 指导修复”的交付型安全工具。
