...
function renderTickets() {
  ...
  tickets.forEach(function(t) {
    ...
    let statusLabel = { pending: '待修复', confirmed: '已确认', applying: '应用中', in_progress: '修复中', fixed: '已修复', failed: '修复失败', rolled_back: '已回滚', ignored: '已忽略' }[t.status] || t.status;
...
function showTicketDetail(id) {
  ...
  let statusLabel = { pending: '待修复', confirmed: '已确认', applying: '应用中', in_progress: '修复中', fixed: '已修复', failed: '修复失败', rolled_back: '已回滚', ignored: '已忽略' }[ticket.status] || ticket.status;
  ...
  html += '<div class="ticket-timeline" id="ticket-timeline-' + ticket.id + '"><div class="ticket-timeline-loading">加载中...</div></div></div>';
  ...
  html += '</select>';
  html += '<button class="ticket-btn primary" onclick="verifyTicket(' + ticket.id + ')">复测验证</button>';
...
function loadTicketTimeline(id) {
  let container = document.getElementById('ticket-timeline-' + id);
  if (!container) return;
  authFetch('/api/fix-tickets/' + id + '/timeline').then(function(r) { return r.json(); }).then(function(data) {
...
function verifyTicket(id) {
  if (!confirm('确定对工单 #' + id + ' 进行复测验证？这将重新扫描目标并对比修复效果。')) return;
  let btn = document.querySelector('.ticket-btn.primary[onclick="verifyTicket(' + id + ')"]');
  if (btn) {
    btn.textContent = '验证中...';
    btn.disabled = true;
  }
  authFetch('/api/fix-tickets/' + id + '/verify', {
    method: 'POST',
